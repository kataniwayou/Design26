...
public class ActivityExecutedEventConsumer : IConsumer<ActivityExecutedEvent>
{
    ...
    public async Task Consume(ConsumeContext<ActivityExecutedEvent> context)
    {
        ...
        _logger.LogInformationWithCorrelation("Processing ActivityExecutedEvent. OrchestratedFlowId: {OrchestratedFlowId}, StepId: {StepId}, ProcessorId: {ProcessorId}, ExecutionId: {ExecutionId}",
            activityEvent.OrchestratedFlowEntityId, activityEvent.StepId, activityEvent.ProcessorId, activityEvent.ExecutionId);
        ...
        try
        {
            ...
            if (nextSteps.Count == 0)
            {
                ...
                _logger.LogInformationWithCorrelation("Flow branch termination detected and processed. OrchestratedFlowId: {OrchestratedFlowId}, StepId: {StepId}, Duration: {Duration}ms",
    ...
    private async Task ProcessSingleNextStepAsync(ActivityExecutedEvent activityEvent, Guid nextStepId, OrchestrationCacheModel orchestrationData)
    {
        ...
        try
        {
            ...
            _logger.LogDebugWithCorrelation("Published ExecuteActivityCommand for next step. NextStepId: {NextStepId}, ProcessorId: {ProcessorId}, AssignmentCount: {AssignmentCount}",
